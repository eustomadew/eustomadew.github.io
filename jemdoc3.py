...
def parseconf(cns):
  ...
  # NEW! fs = [StringIO.StringIO(standardconf())]
  fs = [StringIO(standardconf())]  # StringIO.BytesIO(standardconf())]
  for sname in cns:
...
def doincludes(f, l):
  ...
  if l.startswith(ir):
    ...
    # NEW! f.outf.write(nf.read())
    f.outf.write(nf.read().encode('ascii'))
    nf.close()
  elif l.startswith(i):
...
def geneq(f, eq, dpi, wl, outname):
  ...
  # NEW! g.write(preamble)
  g.write(preamble.encode('ascii'))

  # Write the equation itself.
  if wl:
    # NEW! g.write('\\[%s\\]' % eq)
    g.write(('\\[%s\\]' % eq).encode('ascii'))
  else:
    # NEW! g.write('$%s$' % eq)
    g.write(('$%s$' % eq).encode('ascii'))
